...
5. 组装 Scene Spec：`{version:'1', canvas:{width,height}, parts:[{id, kind, x, y, label?, bindings:[{variable, target?, effect}]}]}`。布局清晰即可（输入左、输出右）。**状态机显示**：若程序含 CASE 状态机且状态变量映射到输出,建议用 `numeric-display`（text 效果）显示当前状态值。如有条件,可附加状态枚举映射注释（如 `0=IDLE, 1=RUNNING, 3=FAULT`）作为 `label`,帮助用户在画面上直读状态含义。
